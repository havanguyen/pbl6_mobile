...
$localReportDir = ".\bao_cao_test\admin"
$xmlReportFile = "$localReportDir\admin_report.xml"
$htmlReportFile = "$localReportDir\admin_report.html"
...
Write-Host "Running Admin Flow Test..." -ForegroundColor Yellow
Write-Host "(Please wait for Flutter to build and run the test...)" -ForegroundColor DarkGray

flutter test integration_test/tests/e2e_admin_flow_test.dart -d $deviceID --machine | dart pub global run junitreport:tojunit --output $xmlReportFile
...
<body>
    <div class="container">
        <header>
            <h1>Admin Flow Integration Test Report</h1>
