...
class AppointmentDetailsSheet extends StatelessWidget {
  ...
  Widget _buildStatusBadge(BuildContext context, CustomThemeExtension theme) {
    ...
    switch (appointment.status) {
      case 'BOOKED':
      case 'RESCHEDULED':
        bgColor = theme.blue.withOpacity(0.1);
        textColor = theme.blue;
        borderColor = theme.blue.withOpacity(0.2);
        statusText = appointment.status == 'RESCHEDULED'
            ? AppLocalizations.of(context).translate('status_rescheduled')
            : AppLocalizations.of(context).translate('status_booked');
        break;
      case 'CONFIRMED':
        bgColor = theme.green.withOpacity(0.1);
        textColor = theme.green;
        borderColor = theme.green.withOpacity(0.2);
        statusText = AppLocalizations.of(context).translate('status_confirmed');
        break;
      case 'COMPLETED':
        bgColor = theme.grey.withOpacity(0.1);
        textColor = theme.grey;
        borderColor = theme.grey.withOpacity(0.2);
        statusText = AppLocalizations.of(context).translate('status_completed');
        break;
      case 'CANCELLED':
      case 'CANCELLED_BY_STAFF':
      case 'CANCELLED_BY_PATIENT':
        bgColor = theme.red.withOpacity(0.1);
        textColor = theme.red;
        borderColor = theme.red.withOpacity(0.2);
        statusText = AppLocalizations.of(context).translate('status_cancelled');
        break;
      case 'NO_SHOW':
        bgColor = theme.yellow.withOpacity(0.1);
        textColor = theme.yellow;
        borderColor = theme.yellow.withOpacity(0.2);
        statusText = AppLocalizations.of(context).translate('status_no_show');
        break;
      default:
        bgColor = theme.muted.withOpacity(0.1);
        textColor = theme.mutedForeground;
        borderColor = theme.muted.withOpacity(0.2);
      ...
      children: [
        ...
        if (appointment.status == 'CONFIRMED')
          _buildActionButton(
            ...
            onPressed: isLoading
                ? null
                : () => AppointmentActionDialogs.showCompleteDialog(
                    context,
                    appointment,
                  ),
          ),
        ...
        _buildActionButton(
          ...
          color: theme.destructive, // or theme.red
          onPressed: isLoading
              ? null
