  ...
  Future<void> _confirmDelete() async {
    ...
    if (mounted) {
      ...
      if (success) {
        ...
        widget.snackbarService.showSuccess('Xóa bài viết thành công!');
      } else {
        setState(() {
          _apiErrorMessage = provider.error ?? 'Xóa thất bại. Kiểm tra mật khẩu hoặc thử lại.';
          provider.clearError();
        });
      }
    }
    ...
    return AlertDialog(
      ...
      title: Row( // Add icon to title
        children: [
          ...
          Text(
            'Xác nhận xóa',
            style: TextStyle(color: context.theme.popoverForeground, fontWeight: FontWeight.bold),
          ),
        ],
      ),
      content: SingleChildScrollView( // Allow scrolling if content overflows
        child: Column(
          ...
          children: [
            Text(
              ...
              style: TextStyle(color: context.theme.popoverForeground.withOpacity(0.8), fontSize: 14),
            ),
            const SizedBox(height: 4),
            Text(
              '"${widget.blog['title']}"?',
              style: TextStyle(color: context.theme.popoverForeground, fontSize: 15, fontWeight: FontWeight.w500),
              maxLines: 2,
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: 'Mật khẩu để xác nhận',
                hintStyle: TextStyle(color: context.theme.mutedForeground.withOpacity(0.5)),
                ...
                errorBorder: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(8),
                    borderSide: BorderSide(color: context.theme.destructive, width: 1.5)),
            ...
            if (_apiErrorMessage != null) ...[
              ...
              Padding( // Add padding for error message
      ...
      actionsPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), // Adjust action button padding
      actions: [
        TextButton(
        ...
        ElevatedButton.icon( // Use ElevatedButton for delete action
            icon: _isDeleting
                ? SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: context.theme.destructiveForeground))
